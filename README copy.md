...
### ⛓️ Ledger Integrity Chain (Blockchain-Style Audit Trail)

A distinguishing, currently-undocumented-elsewhere piece of the system: every approved CSG project gets a **SHA-256 linked hash chain** (`BlockchainService`, `Chain` model) that records a tamper-evident audit trail of its financial ledger entries.

- **Genesis block**: created on project approval, hashing a snapshot of the project's core data (title, description, budget, approval timestamp).
- **Linked blocks**: every subsequent ledger entry hashes its own data together with the previous block's hash (`prev_hash`), so altering any historical entry invalidates every hash after it.
- **Verification endpoint**: `BlockchainController` exposes a chain viewer and an integrity-check action that recomputes hashes and reports whether the chain is intact — surfaced to CSG officers (their own projects) and Advisers/Superadmin (all projects).
- **Scope note**: this is a single-writer, application-level hash chain for internal auditability, not a distributed/consensus blockchain — worth stating explicitly so the term isn't over-read by reviewers expecting a DLT.

### 🏆 Gamification

- Points system tied to student/teacher engagement (ratings submitted, participation)
...
STEP has **six** distinct role slugs, not a simple four-tier hierarchy. Access is enforced in two layers:

1. **Route-level (`role:` middleware)** — coarse gating on entire route groups (`routes/web.php`), e.g. `role:superadmin`, `role:admin,admin-sadu`, `role:csg`, `role:student,teacher`.
2. **Grant-level (`RolePermissionService` + `permission` / `role_permission` tables)** — a seeded catalog of module.action permissions (`projects.approve`, `ledger.submit`, `ratings.moderate`, etc.) that Superadmin can edit per-role (and per CSG council **position**) from `/sadmin/roles`, without touching code.

This two-layer design means the matrix below reflects what's actually enforced today; it is **not** meant to be a comprehensive claim of a generic LMS "grades/courses" system — STEP does not currently model grades or course creation at all, despite earlier documentation implying it did.
...
│  Full catalog access. Only role that can:                  │
│  manage users, edit role/permission grants, view            │
│  system-wide audit logs, configure CSG positions/terms.     │
└───────────────────────────────────────────────────────────┘
                              ▲
                ┌─────────────┴─────────────┐
                ▼                           ▼
   ┌───────────────────────┐   ┌───────────────────────────┐
   │ Council Adviser (admin) │   │ SADU Admin (admin-sadu)   │
   │ Approve projects/ledger/│   │ Same grant set as Adviser,│
   │ meeting minutes; view-  │   │ scoped to Student Affairs │
   │ only on ratings         │   │ & Discipline oversight    │
   └───────────────────────┘   └───────────────────────────┘
                              ▲
                              │
                 ┌────────────┴────────────┐
                 ▼                         ▼
        ┌─────────────────┐      ┌─────────────────────┐
        │   CSG Officer    │      │  Student / Teacher   │
        │  (csg, per-       │      │  (student, teacher)  │
        │  position grants) │      │                      │
        │  Create/edit/     │      │  View projects and   │
        │  delete projects, │      │  meetings, submit    │
        │  ledger entries,  │      │  ratings, track       │
        │  meetings, proof  │      │  points/badges/       │
        │  docs (pending    │      │  leaderboard          │
        │  adviser approval)│      │                      │
        └─────────────────┘      └─────────────────────┘
```
...
> Reflects the default grants seeded by `RolePermissionService::defaultGrants()`. Superadmin can customize these per-role and per-CSG-position at runtime — this table shows the **defaults**, not a hardcoded ceiling.
...
| View blockchain/ledger integrity chain | ❌ | ❌ | ✅ (own projects) | ✅ | ✅ |
| Manage users (create, disable, reset password, change role) | ❌ | ❌ | ❌ | ❌ | ✅ |
| Edit role/permission grants & CSG positions | ❌ | ❌ | ❌ | ❌ | ✅ |
| View system-wide audit logs | ❌ | ❌ | ❌ | ❌ | ✅ |
| System configuration / settings | ❌ | ❌ | ❌ | ❌ | ✅ |

**Notes / limitations worth knowing before relying on this table:**
- Several `/sadmin/*` pages referenced in the routes (e.g. Data Backup, Organizations, System Settings, Global Reports, Engagement Rules) currently render static Inertia pages without a backing controller — they're scaffolded UI, not yet functional endpoints. Treat them as roadmap, not shipped features.
- The route-level `role:superadmin` group is annotated in the code itself as **"Temporarily without middleware for testing"** — worth confirming that hardening is finished before any production/public deployment.
- CSG grants can additionally be scoped **per council position** (e.g. President vs. Treasurer) via the `role_permission.position_id` column, which isn't reflected in the table above since it's configured at runtime rather than as a fixed default.
...
Being upfront about the current state of the repo, since transparency about known gaps is part of what makes a codebase trustworthy to other engineers:

- **Secrets hygiene**: a non-`.env`-named environment file and a captured session-cookie file have previously been committed to this repo. If you're standing this project up yourself, generate your own credentials from scratch (`.env.example` → `.env`), never commit populated env files under any filename, and rely on `.gitignore`'s `.env` rule only for files actually named `.env*`.
- **Duplicate/scratch files**: the repo currently carries several `copy`-suffixed duplicates (`composer copy.json`, `composer - Copy.lock`, `routes/web copy.php`, `BulkRegistrationController copy.php`) and multiple historical SQL dumps (`step_system_database*.sql`). These are being cleaned up; if you find one still present, treat the non-`copy` file as canonical.
- **Scaffolded pages**: some `/sadmin/*` routes (Data Backup, Organizations, System Settings, Global Reports, Engagement Rules, Master Data) currently return static placeholder views rather than wired-up functionality — flagged in the permission matrix above.
- **Legacy/duplicate models**: a few models exist in both a namespaced form and a flat, numerically-suffixed form (e.g. `User\Rating` vs. `Rating1`, `User\Project` vs. `Project1`). Consolidation is planned; new work should prefer the namespaced versions under `App\Models\User\*` and `App\Models\CSG\*`.
- **Repo size**: demo media (`public/videos`, `public/images`) is currently committed directly rather than via Git LFS or external hosting, which inflates clone size. Migrating this is on the roadmap.
